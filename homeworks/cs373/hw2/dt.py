...
class Leaf(Node):
    ...
    def predict_class_probabilities(self, X: ArrayLike) -> np.ndarray:
        ...
        for i in self.class_labels:
            p = self.class_probabilities[i]
            myArr.append(p)
        probabilities = np.array([myArr])
    ...
    def predict(self, X: ArrayLike) -> np.ndarray:
        ...
        max_ind = np.argmax(probabilities, axis = 1)
        for i in max_ind:
            label = self.class_labels[i]
            myArr.append(label)
            
        labels = np.array(myArr)
        # <<< END OF YOUR CODE <<<
        return labels
...
class DecisionTree:
    ...
    def _build_tree(self, X: ArrayLike, y: ArrayLike, max_depth: int, exclude: set = set()) -> Node:
        ...
        if max_depth <= 0 or self._is_pure(y) or len(exclude) == X.shape[1]:
            ...
            leaf_node = Leaf(self.scorer.compute_class_probabilities(y))
            return leaf_node
        ...
        feature_of_split = None
        splits = {}
        for i in range(num_classes):
            if i not in exclude:
                info_gain = self.scorer.information_gain(X, y, i)
                if info_gain > max_gain:
                    max_gain = info_gain
                    feature_of_split = i
        if feature_of_split is not None:
            feature_to_split = Split(feature_of_split, {})
            indexes = feature_to_split._choose_branch(X)
            for i, j in indexes.items():
                X_split = X[j]
                y_split = y[j]
                splits[i] = (X_split, y_split)
            ...
            if 'NA' not in children:
                ...
                probabilities = 1.0 / len(unique_labels)
                class_probabilities = {}
                for i in unique_labels:
                    class_probabilities[i] = probabilities
                leaf_node = Leaf(class_probabilities)
                children["NA"] = leaf_node
        ...
        # Return a split node.
        # >>> YOUR CODE HERE >>>
        if feature_of_split is None:
            leaf_node = Leaf(self.scorer.compute_class_probabilities(y))
            return leaf_node
        split_node = Split(feature_of_split, children)
        return split_node
        # <<< END OF YOUR CODE <<<
