...
class NaiveBayes:
    ...
    def compute_class_probabilities(self, y_train: np.ndarray) -> Dict[Any, float]:
        ...
        for i in class_probabilities:
            class_probabilities[i] = (class_probabilities[i] + self.alpha) / (len(y_train) + self.alpha * len(classes))
    ...
    def compute_feature_probabilities(self, X_j_train: np.ndarray, y_train: np.ndarray) -> Dict[Tuple[Any, Any], float]:
        ...
        for c in class_labels:
            ...
            counter = np.sum(y_train == c)

            for i in unique_features:
                numerator = np.sum((X_j_train == i) & (y_train == c)) + self.alpha
                denominator = counter + self.alpha * len(unique_features)
                laplace = numerator / denominator

                feature_probabilities[(i, c)] = laplace
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray) -> None:
        ...
        for i in range(n_features):
            subset_data = X_train[:, i]
            self.feature_probabilities[i] = self.compute_feature_probabilities(subset_data, y_train)
        # <<< END OF YOUR CODE <<<
        ...
        for i in range(n):
            ...
            if total <= 0:
                ...
                for j in range(num_labels):
                    probabilities[i, j] = probabilities[i, j] * factor
    ...
    def predict(self, probabilities: np.ndarray) -> np.ndarray:
        ...
        y_pred = self.class_labels[max_indexes]
        m = y_pred.shape[0]
        y_pred.reshape(m, 1)
        # <<< END OF YOUR CODE <<<
        return y_pred
    ...
    def evaluate(self, y_test: np.ndarray, probabilities: np.ndarray) -> Tuple[float, float]:
        ...
        zero_one_loss =  num_incorrect / num_predictions

        for i in range(num_predictions):
            true_point = np.where(self.class_labels == y_test[i])[0]
            true_point = true_point[0]
            true_prob = probabilities[i, true_point]
            squared_loss += (1 - true_prob) ** 2
        
        squared_loss = squared_loss / num_predictions
